...
through `tab_style`, not a same-call `tab_options()` kwarg. `gt_check.py`'s
`heading-band` check (`scripts.md`) verifies the rendered **result** — white
column-label text — not this specific mechanism: a table relying purely on
auto-contrast (no `tab_style` call) already passes it too. Pin it explicitly anyway,
