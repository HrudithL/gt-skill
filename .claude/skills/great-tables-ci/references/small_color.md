...
**branding tier + neutral-grey** palette below — never an improvised saturated color.

All hexes are inlined here (mirrors `references/palettes.md` §0 for the branding
tier and §2 for neutrals), so this file is self-contained.
...
there, not the subtitle) so the number is reproducible. Do
...
competing full heatmap fill. This is a taste-level call, not a hard count: a table can
legitimately carry more than 2 full fills when every one of them is genuinely
load-bearing to the request, and it can also carry just 1 when that's all the data
supports — the point is that once color starts competing with itself for the reader's
attention, step down to the lighter technique instead of adding another fill. Don't
build a "top N" / "bottom N" extreme-cell selection mechanic for this — the step-down
happens at the level of a whole measure, not a subset of its cells.
