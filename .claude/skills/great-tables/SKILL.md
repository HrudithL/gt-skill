...
- **Before you finalize column order** (Step 2): the column (or column-group) carrying
  the primary/most-important heatmap fill belongs at an outer edge — right after the
  stub, or as the last column(s) — never buried in the middle. Columns providing
  context/inputs a reader needs first precede columns reporting a derived/resulting
  outcome, so an outcome-type measure naturally lands at the right edge while a
  measure that IS the subject's defining fact lands at the left edge. Use
