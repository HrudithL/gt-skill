...
SKILL_LABEL = "with skill (3 samples)"
...
def plot_comparator_score(
    ...
    plot = (
        ...
        + ylim(0, 100)
        + labs(
            title="Evaluation score: repeated samples with the skill vs. without",
            subtitle=lift_line,
            caption=(
                "Each sample is one full agent invocation on the prompt. "
                "Box = spread of the skill's score across 3 independent samples on the same prompt -- "
                "shorter is more consistent. Dot = a single unassisted baseline sample "
                "(agent run without the skill)."
            ),
            x="",
            y="Evaluation score (%)",
        )
