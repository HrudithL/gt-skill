...
DIMENSIONS: dict[str, dict] = {
    ...
    "color_restraint_quality": {
        ...
        "assesses": (
            "Applicability for this dimension is decided by the harness "
            "-- a deterministic count of distinct per-value color fills "
            "computed from the candidate's own source, not from the "
            "rendered image -- before you are ever asked. You are only "
            "ever sent this dimension when that count already qualifies, "
            "so always treat it as applicable and never self-report "
            "`applicable=false` for it; see the Applicability section "
            "below. When scoring: judge ONLY genuine per-value, "
            "data-driven fills (e.g. a `data_color`/heatmap-style "
            "treatment on a specific measure column) -- ignore the "
            "heading band, stub tint, row striping, and any summary-row "
            "shading entirely; those are structural/branding surfaces, "
            "never per-value measure fills. Given the fills that are "
            "present, does the candidate favor bold text or text color "
            "-- rather than yet another full fill -- for any measure "
            "that is clearly secondary or incidental to the table's "
            "evident main subject, so the measure(s) that actually carry "
            "the table's main story stay visually distinguishable? Do "
            "not penalize a candidate for having several colored "
            "measures if each one is plausibly part of the table's main "
            "subject -- judge whether any fill specifically looks "
            "incidental/secondary to the table's evident focus, not how "
            "many fills exist. This is a light, qualitative judgment "
            "call about restraint: the count above only gates whether "
            "you're asked at all, never the score itself."
        ),
        "anchors": {
            1: (
                "Multiple measures already carry a full per-value color "
                "fill, and at least one of those fills is clearly on a "
                "measure that is secondary or incidental to what the "
                "table is evidently about (not one of the table's "
                "main-subject measures) -- no restraint exercised on the "
                "incidental one."
            ),
            3: (
                "Ambiguous or partial: most fills are plausibly on "
                "main-subject measures, but at least one fill is "
                "questionable/borderline secondary, or restraint is "
                "applied inconsistently across similar measures."
            ),
            5: (
                "Every full color fill present is plausibly on a "
                "measure central to the table's evident main subject, "
                "however many there are -- this dimension does NOT "
                "penalize raw fill count -- and any genuinely "
                "secondary-but-notable measure is carried by bold text "
                "or text color instead of an additional fill."
            ),
        },
    },
}
